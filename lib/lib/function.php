...
class login_function
{
   private $con;
	...
	{
		$this->con = new mysqli("localhost","root","","dtmanager");
	...
	{	
		...
		{	
			...
			{							
				$date = date("Y-m-d");
				$time = date("H-i-s A");
				if($stmt_insert = $this->con->prepare("INSERT INTO `tmp_invoice`(`customer_id`, `service`, `actual_amount`, `discount_amount`) VALUES (?,?,?,?)"))
				{
					$stmt_insert->bind_param("ssss",$c_id,$service,$actual_amount,$discount_amount);
