...
$db		=	new login_function();
	...
	if(isset($_POST['add_btn']))
	{
		...
		if($flag==0)
		{
			$db->update_bank_details($up_id,$b_name,$acc_name,$acc_no,$ifsc_code,$branch_name,$other_info);
			$succ_flag = 1 ;
			
		}
	}
	$report_details = $db->get_all_bank_information_by_id($up_id);
	if(!empty($report_details))
	{
		$counter =0;
										
		$id				=	$report_details[$counter][0];
		$b_name			=	$report_details[$counter][1];
		$acc_name		=	$report_details[$counter][2];
		$acc_no			=	$report_details[$counter][3];
		$ifsc_code		=	$report_details[$counter][4];
		$branch_name	=	$report_details[$counter][5];
		$other_info 	=	$report_details[$counter][6];
	}
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="ibox" style="border-radius:5px; padding:7px;">
	 <form class="form-pink" method="post" action="<?php echo $_SERVER['PHP_SELF']?>" name="myForm" onsubmit="return validateForm()" autocomplete="off" enctype="multipart/form-data">
		...
		<div class="ibox-head">
			<div class="ibox-title"><i class="fas fa-user-tie" style="margin-right:10px;"></i>Update BANK DETAILS</div>
		</div>
			...
			<div class="row">
				...
				<div class="col-sm-12 form-group mb-12" style="text-align:center; padding-left:0px; padding-right:0px; padding-top:20px;">
					<div class="col-sm-4 form-group mb-4" style="margin:auto;">
						<button class="btn btn-pink btn-air" type="submit" name="add_btn" style="width:100%;">UPDATE BANK DETAILS</button>
					</div>
				</div>
			</div>
			
		</div>
		<center><a href="bank-details-report.php" style="color:red;font-weight:bold;">Back To List</a></center>
	</form>
	</div>
</div>
</div>
